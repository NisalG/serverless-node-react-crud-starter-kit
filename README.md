...
* [serverless-plugin-split-stacks](https://www.serverless.com/plugins/serverless-plugin-split-stacks)
    ...
    * This will create Nested Stacks in AWS Systems Manager > Application Manager > CloudFormation stacks
        * E.g: `aws-serverless-typescript-crud1-dev-1NestedStack-1A8W5VSDFWURT`
        ...
        * Add login function
            ...
            * `adminInitiateAuth`
                ...
                * `AuthParameters`
                    * `UserName`
                    * `Password`
            * Use Postman [[http://localhost:3000/dev/v1/cartAuth/authenticateUser](http://localhost:3000/dev/v1/cartAuth/authenticateUser)] and login will respond with `AccessToken `etc.

    Amazon Cognito generates JWTs but also adds additional claims to JSON Web Tokens, 
    ...
    * [Husky NPM](https://www.npmjs.com/package/husky) | [Husky Docs](https://typicode.github.io/husky/)
        ...
        * Commands in the .husky folder will run on the commit and push events.
            * .husky\pre-commit
            ...
            ```
            # npm run prettier
            npm run compile #(or npm run tsc to compile  
            TSC and build .lib folder 
            content files)
...
* Mongoose error: {
    	...
    	"error": {
        	...
        	"message": "Cannot populate path `items` because it is not in your schema. Set the `strictPopulate` option to false to override."
